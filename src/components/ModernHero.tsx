...
const ModernHero = ({ banners = [] }: { banners?: Banner[] }) => {
  ...
  // Map API banners to the hero structure, or fallback to defaults
  const heroSlides = banners && banners.length > 0
    ? banners.map(banner => ({
      title: banner.title,
      highlight: "", // Highlight can be extracted or optional
      subtitle: banner.subtitle || "Premium Quality Products",
  ...
  useEffect(() => {
    ...
    // Reset slide index if it's out of bounds after a data update
  ...
  const currentHero = heroSlides[currentSlide] || defaultSlides[0];
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-tight mb-4 md:mb-6 px-2 sm:px-0">
            {currentHero.highlight ? (
              <>
                ...
                <span className="gradient-text tracking-tighter">{currentHero.highlight}</span>
              </>
            ) : (
              <span className="leading-tight block max-w-4xl mx-auto">{currentHero.title}</span>
            )}
          </h1>
